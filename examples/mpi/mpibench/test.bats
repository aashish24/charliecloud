...
@test "$ch_tag/pingpong (guest launch)" {
    ...
    run ch-run "$ch_img" -- mpirun $chtest_mpirun_np "$imb_mpi1" $imb_args PingPong
    echo "$output"
    [[ $status -eq 0 ]]
    check_errors "$output"
...
@test "$ch_tag/sendrecv (guest launch)" {
    # shellcheck disable=SC2086
    run ch-run "$ch_img" -- mpirun $chtest_mpirun_np "$imb_mpi1" $imb_args Sendrecv
    echo "$output"
    [[ $status -eq 0 ]]
    check_errors "$output"
...
@test "$ch_tag/sendrecv (host launch)" {
    multiprocess_ok
    # shellcheck disable=SC2086
    run $mpirun_core ch-run --join "$ch_img" -- "$imb_mpi1" $imb_args Sendrecv
    echo "$output"
    [[ $status -eq 0 ]]
    check_errors "$output"
...
@test "$ch_tag/allreduce (guest launch)" {
    # shellcheck disable=SC2086
    run ch-run "$ch_img" -- mpirun $chtest_mpirun_np "$imb_mpi1" $imb_args Allreduce
    echo "$output"
    [[ $status -eq 0 ]]
    check_errors "$output"
    check_process_ct "$chtest_cores_node" "$output"
...
@test "$ch_tag/allreduce (host launch)" {
    multiprocess_ok
    # shellcheck disable=SC2086
    run $mpirun_core ch-run --join "$ch_img" -- "$imb_mpi1" $imb_args Allreduce
    echo "$output"
    [[ $status -eq 0 ]]
    check_errors "$output"
    check_process_ct "$chtest_cores_total" "$output"
