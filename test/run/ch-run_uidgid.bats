...
@test 'user and group as specified' {
    g=$(ch-run $uid_args $gid_args "$ch_chtest_img" -- id -un)
    [[ $GUEST_USER = "$g" ]]
    g=$(ch-run $uid_args $gid_args "$ch_chtest_img" -- id -u)
    [[ $guest_uid = "$g" ]]
    g=$(ch-run $uid_args $gid_args "$ch_chtest_img" -- id -gn)
    [[ $GUEST_GROUP = "$g" ]]
    g=$(ch-run $uid_args $gid_args "$ch_chtest_img" -- id -g)
    [[ $guest_gid = "$g" ]]
}

@test 'chroot escape' {
    # Try to escape a chroot(2) using the standard approach.
    ch-run $uid_args $gid_args "$ch_chtest_img" -- /test/chroot-escape
}

@test '/dev /proc /sys' {
    # Read some files in /dev, /proc, and /sys that I shouldn't have access to.
    ch-run $uid_args $gid_args "$ch_chtest_img" -- /test/dev_proc_sys.py
}
    ...
    for d in $CH_TEST_PERMDIRS; do
        ...
        echo "verifying: $d"
          ch-run --no-home --private-tmp \
                 $uid_args $gid_args -b "$d" "$ch_chtest_img" -- \
...
@test 'mknod(2)' {
    ...
    ch-run $uid_args $gid_args "$ch_chtest_img" -- \
...
@test 'privileged IPv4 bind(2)' {
    ...
    ch-run $uid_args $gid_args "$ch_chtest_img" -- /test/bind_priv.py $addrs
}
...
@test 'remount host root' {
    ...
    run ch-run $uid_args $gid_args "$ch_chtest_img" -- \
...
@test 'setgroups(2)' {
    ...
    ch-run $uid_args $gid_args "$ch_chtest_img" -- /test/setgroups
}

@test 'seteuid(2)' {
    # Try to seteuid(2) to another UID we shouldn't have access to
    ch-run $uid_args $gid_args "$ch_chtest_img" -- /test/setuid
}
...
@test 'signal process outside container' {
    ...
    ch-run $uid_args $gid_args "$ch_chtest_img" -- /test/signal_out.py
}
