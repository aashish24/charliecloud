...
@test 'relative path to image' {  # issue #6
    scope quick
    cd "$(dirname "ch_chtest_img")" && ch-run "$(basename "$ch_chtest_img")" -- true
}

@test 'symlink to image' {  # issue #50
    scope quick
    ln -sf "$ch_chtest_img" "$BATS_TMPDIR/symlink-test"
    ch-run "$BATS_TMPDIR/symlink-test" -- true
}

@test 'mount image read-only' {
    scope quick
    run ch-run "$ch_chtest_img" sh <<EOF
set -e
...
@test 'mount image read-write' {
    scope quick
    ch-run -w "$ch_chtest_img" -- sh -c 'echo writable > write'
    ch-run -w "$ch_chtest_img" rm write
}

@test '/usr/bin/ch-ssh' {
    scope quick
    ls -l "$ch_bin/ch-ssh"
    ch-run "$ch_chtest_img" -- ls -l /usr/bin/ch-ssh
    ch-run "$ch_chtest_img" -- test -x /usr/bin/ch-ssh
    host_size=$(stat -c %s "$ch_bin/ch-ssh")
    guest_size=$(ch-run "$ch_chtest_img" -- stat -c %s /usr/bin/ch-ssh)
...
@test '$HOME' {
    ...
    # default: set $HOME
    # shellcheck disable=SC2016
    run ch-run "$ch_chtest_img" -- /bin/sh -c 'echo $HOME'
    echo "$output"
    [[ $status -eq 0 ]]
    [[ $output = /home/$USER ]]
    ...
    # shellcheck disable=SC2016
    run ch-run --no-home "$ch_chtest_img" -- /bin/sh -c 'echo $HOME'
    echo "$output"
    [[ $status -eq 0 ]]
    [[ $output = "$HOME" ]]
...
@test '$PATH: add /bin' {
    ...
    echo "$PATH2"
    # shellcheck disable=SC2016
    PATH=$PATH2 run ch-run "$ch_chtest_img" -- /bin/sh -c 'echo $PATH'
    echo "$output"
    [[ $status -eq 0 ]]
    [[ $output = "$PATH2" ]]
...
@test 'ch-run --cd' {
    ...
    run ch-run "$ch_chtest_img" -- pwd
    echo "$output"
    [[ $status -eq 0 ]]
    [[ $output = '/' ]]

    # Specify initial working directory.
    run ch-run --cd /dev "$ch_chtest_img" -- pwd
    echo "$output"
    [[ $status -eq 0 ]]
    [[ $output = '/dev' ]]

    # Error if directory does not exist.
    run ch-run --cd /goops "$ch_chtest_img" -- true
    echo "$output"
    [[ $status -eq 1 ]]
...
@test 'ch-run --bind' {
    ...
    ch-run -b "$ch_imgdir/bind1" "$ch_chtest_img" -- cat /mnt/0/file1
    # one bind, explicit destination
    ch-run -b "$ch_imgdir/bind1:/mnt/9" "$ch_chtest_img" -- cat /mnt/9/file1

    # two binds, default destination
    ch-run -b "$ch_imgdir/bind1" -b "$ch_imgdir/bind2" "$ch_chtest_img" \
           -- cat /mnt/0/file1 /mnt/1/file2
    # two binds, explicit destinations
    ch-run -b "$ch_imgdir/bind1:/mnt/8" -b "$ch_imgdir/bind2:/mnt/9" "$ch_chtest_img" \
           -- cat /mnt/8/file1 /mnt/9/file2
    # two binds, default/explicit
    ch-run -b "$ch_imgdir/bind1" -b "$ch_imgdir/bind2:/mnt/9" "$ch_chtest_img" \
           -- cat /mnt/0/file1 /mnt/9/file2
    # two binds, explicit/default
    ch-run -b "$ch_imgdir/bind1:/mnt/8" -b "$ch_imgdir/bind2" "$ch_chtest_img" \
           -- cat /mnt/8/file1 /mnt/1/file2

    # bind one source at two destinations
    ch-run -b "$ch_imgdir/bind1:/mnt/8" -b "$ch_imgdir/bind1:/mnt/9" "$ch_chtest_img" \
           -- diff -u /mnt/8/file1 /mnt/9/file1
    # bind two sources at one destination
    ch-run -b "$ch_imgdir/bind1:/mnt/9" -b "$ch_imgdir/bind2:/mnt/9" "$ch_chtest_img" \
           -- sh -c '[ ! -e /mnt/9/file1 ] && cat /mnt/9/file2'

    # omit tmpfs at /home, which shouldn't be empty
    ch-run --no-home "$ch_chtest_img" -- cat /home/overmount-me
    # overmount tmpfs at /home
    ch-run -b "$ch_imgdir/bind1:/home" "$ch_chtest_img" -- cat /home/file1
    # bind to /home without overmount
    ch-run --no-home -b "$ch_imgdir/bind1:/home" "$ch_chtest_img" -- cat /home/file1
    # omit default /home, with unrelated --bind
    ch-run --no-home -b "$ch_imgdir/bind1" "$ch_chtest_img" -- cat /mnt/0/file1
}
...
@test 'ch-run --bind errors' {
    ...
    echo "$output"
    [[ $status -eq 1 ]]
    r="can't bind .+/bind1 to $ch_chtest_img/mnt/10: No such file or directory"
    [[ $output =~ $r ]]

    # no argument to --bind
    run ch-run "$ch_chtest_img" -b
    echo "$output"
    [[ $status -eq 64 ]]
    [[ $output =~ 'option requires an argument' ]]

    # empty argument to --bind
    run ch-run -b '' "$ch_chtest_img" -- true
    echo "$output"
    [[ $status -eq 1 ]]
    [[ $output =~ '--bind: no source provided' ]]

    # source not provided
    run ch-run -b :/mnt/9 "$ch_chtest_img" -- true
    echo "$output"
    [[ $status -eq 1 ]]
    [[ $output =~ '--bind: no source provided' ]]

    # destination not provided
    run ch-run -b "$ch_imgdir/bind1:" "$ch_chtest_img" -- true
    echo "$output"
    [[ $status -eq 1 ]]
    [[ $output =~ '--bind: no destination provided' ]]

    # source does not exist
    run ch-run -b "$ch_imgdir/hoops" "$ch_chtest_img" -- true
    echo "$output"
    [[ $status -eq 1 ]]
    r="can't bind .+/hoops to $ch_chtest_img/mnt/0: No such file or directory"
    [[ $output =~ $r ]]

    # destination does not exist
    run ch-run -b "$ch_imgdir/bind1:/goops" "$ch_chtest_img" -- true
    echo "$output"
    [[ $status -eq 1 ]]
    r="can't bind .+/bind1 to $ch_chtest_img/goops: No such file or directory"
    [[ $output =~ $r ]]

    # neither source nor destination exist
    run ch-run -b "$ch_imgdir/hoops:/goops" "$ch_chtest_img" -- true
    echo "$output"
    [[ $status -eq 1 ]]
    r="can't bind .+/hoops to $ch_chtest_img/goops: No such file or directory"
    [[ $output =~ $r ]]

    # correct bind followed by source does not exist
    run ch-run -b "$ch_imgdir/bind1:/mnt/9" -b "$ch_imgdir/hoops" "$ch_chtest_img" -- true
    echo "$output"
    [[ $status -eq 1 ]]
    r="can't bind .+/hoops to $ch_chtest_img/mnt/1: No such file or directory"
    [[ $output =~ $r ]]

    # correct bind followed by destination does not exist
    run ch-run -b "$ch_imgdir/bind1" -b "$ch_imgdir/bind2:/goops" "$ch_chtest_img" -- true
    echo "$output"
    [[ $status -eq 1 ]]
    r="can't bind .+/bind2 to $ch_chtest_img/goops: No such file or directory"
    [[ $output =~ $r ]]
}
